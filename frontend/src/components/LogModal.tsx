...
export default function LogModal({ isOpen, onClose, onLogAdded, isPast }: any) {
  ...
  const [studyHours, setStudy] = useState(0);
  const [screenTime, setScreen] = useState(0);
  const [sleepHours, setSleep] = useState(0);
  const [codingHours, setCoding] = useState(0);
  const [mood, setMood] = useState(5);
  const [isSubmitting, setSubmitting] = useState(false);

  // Reset date when modal opens based on isPast prop
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const payload = { studyHours, screenTime, sleepHours, codingHours, mood, date };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min="0" step="0.5" className="w-full bg-black/40 border border-white/10 rounded-lg p-2 text-white" value={studyHours} onChange={e => setStudy(Number(e.target.value))} />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Coding Hours</label>
              <input type="number" min="0" step="0.5" className="w-full bg-black/40 border border-white/10 rounded-lg p-2 text-white" value={codingHours} onChange={e => setCoding(Number(e.target.value))} />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Screen Time</label>
              <input type="number" min="0" step="0.5" className="w-full bg-black/40 border border-white/10 rounded-lg p-2 text-white" value={screenTime} onChange={e => setScreen(Number(e.target.value))} />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Sleep Hours</label>
              <input type="number" min="0" step="0.5" className="w-full bg-black/40 border border-white/10 rounded-lg p-2 text-white" value={sleepHours} onChange={e => setSleep(Number(e.target.value))} />
            </div>
          </div>
